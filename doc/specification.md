...
「らららプログラミング」では、3以上の入力を持つ演算子を多入力演算子と呼びます。
テキストでのプログラミング言語におけるシンタックスシュガーに相当する機能です。
...
実行時間が数十秒以上など、長くかかる場合、前述のプルダウンメニューから実行を打ち切ることができます。
(テキストの出力を数百回以上繰り返すなどにより、実行に使用する時間が増えます)
